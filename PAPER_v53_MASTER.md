...
**Ming-Hung Sung** [ORCID: [0009-0003-3305-0637](https://orcid.org/0009-0003-3305-0637)] (Dual-Loop OS Lab, Taichung, Taiwan)  
**Shih-Yu Sung** (Dual-Loop OS Lab, Lake City, SC, USA)  
...
5. **Pre-Transition Predictive Safety Shield**: Pre-sintered threshold $\epsilon_{\mathrm{sinter}} = \epsilon + K_{\max}^2 \delta_{\mathrm{drift}} + K_{\max} \frac{\xi_{\max}}{\|a_{k^*}\|}$, delivering formal discrete-time forward invariance (Theorem 1B), 2-step finite emergency restoration (Theorem 1C), and SysML v2.0 formal requirement traceability (Lemma 0).
...
The pre-sintered threshold is defined as $\epsilon_{\mathrm{sinter}} \equiv \epsilon + K_{\max}^2 \delta_{\mathrm{drift}} + K_{\max} \frac{\xi_{\max}}{\|a_{k^*}\|}$.
...
$$B(S_0) \ge \epsilon_{\mathrm{sinter}} = \epsilon + K_{\max}^2 \delta_{\mathrm{drift}} + K_{\max} \frac{\xi_{\max}}{\|a_{k^*}\|} > \epsilon$$
...
- **Case 2: Candidate Action Rejected ($k < K_{\max}$)**  
  ...
  &\ge \left( \epsilon + K_{\max}^2 \delta_{\mathrm{drift}} + K_{\max} \frac{\xi_{\max}}{\|a_{k^*}\|} \right) - \left( k^2 \delta_{\mathrm{drift}} + k \frac{\xi_{\max}}{\|a_{k^*}\|} \right) \\
  &= \epsilon + (K_{\max}^2 - k^2)\delta_{\mathrm{drift}} + (K_{\max} - k)\frac{\xi_{\max}}{\|a_{k^*}\|}
  \end{aligned}$$
  Since $k \le K_{\max}-1 < K_{\max}$, both $(K_{\max}^2 - k^2) > 0$ and $(K_{\max} - k) > 0$. Therefore, $B(S_{t+1}) \ge \epsilon$ is strictly satisfied.
...
2. **Conservative Over-Pruning Margin ($\mathrm{OPR} \approx 0.6\%$)**:  
   ...
   $$\epsilon_{\mathrm{sinter}} = \epsilon + K_{\max}^2 \delta_{\mathrm{drift}} + K_{\max}\frac{\xi_{\max}}{\|a_{k^*}\|} = 4.72^\circ > \epsilon = 3.50^\circ$$
...
- **Author Identity & Persistent Metadata**: Ming-Hung Sung (ORCID: [0009-0003-3305-0637](https://orcid.org/0009-0003-3305-0637)). Contact: `sdr.msung@gmail.com`.
